...
public class WebSocketServiceImpl implements WebSocketService {
    ...
    public CompletableFuture<Void> sendFile(String transferCode, byte[] fileData, String fileName, long fileSize) {
        logger.info("Sending file: {} ({} bytes) for transfer code: {}", fileName, fileSize, transferCode);
    ...
    private void sendFileInChunks(String transferCode, byte[] fileData, String fileName, long fileSize, CompletableFuture<Void> future) {
        try {
    ...
    public void sendProgress(String transferCode, double progress, long bytesTransferred) {
        ...
        if (callback != null) {
            WebSocketService.TransferProgress progressInfo = new WebSocketService.TransferProgress(transferCode, progress, bytesTransferred);
    ...
    public void sendTransferComplete(String transferCode, boolean success, String errorMessage) {
        ...
        if (callback != null) {
            WebSocketService.TransferComplete completeInfo = new WebSocketService.TransferComplete(transferCode, success, errorMessage);
